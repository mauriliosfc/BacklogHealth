...
| 162 | `electron/preload.js` com `contextBridge` e `contextIsolation: true` | Segurança Electron — nunca usar `nodeIntegration: true`; toda comunicação renderer→main via `ipcRenderer` exposto pelo preload |
...
| 164 | `nativeTheme.on('updated')` + `ipcMain.on('theme-changed')` para overlay da titlebar | OS-level usa `nativeTheme`; toggle interno envia `theme-changed` via IPC — ambos chamam `win.setTitleBarOverlay(colors)` |
...
| 166 | Padrão de view: cada `open*` esconde todos os siblings (`#tc-view`, `#dp-view`, `#sn-view`) | `openDeliveryPlan` esconde `#tc-view`; `openTeamCapacity` esconde `#dp-view`; `showDashboardView` esconde ambos — manter consistente ao adicionar novas views |
| 167 | `electron/updater.js` com native `https` sem `electron-updater` | Sem code signing disponível; GitHub API retorna nomes de asset com pontos (`Backlog.Health.Setup.x.x.x.exe`) — filtro `/setup/i.test(a.name)` continua funcionando |
| 168 | Queries SN usam midnight BRT local via `new Date(y, m-1, d, h, min, sec).toISOString()` | `"YYYY-MM-DDT00:00:00Z"` é midnight UTC — no fuso BRT (UTC-3) corresponde às 21h do dia anterior; divergência de 3h causava contagens distintas das exibidas no SN e nas extrações Excel |
| 169 | Encerramento de incidente = apenas `resolved_at`; `closed_at` ignorado em todos os cálculos | Incidentes têm dois marcos: `resolved_at` (agente resolveu) e `closed_at` (usuário confirmou ou sistema fechou automaticamente); contar `resolved_at OR closed_at` causava double-count; `closed_at` sem `resolved_at` não representa trabalho concluído; regra aplicada em 5 locais: `incClosedQuery`, `incClosedQ` do histórico, `incAvgResolutionDays`, `fetchSnIncidentBacklog` mode='closed' e `calcMttrByPriority` |
| 170 | Meses configuráveis por instância de gráfico (`chart.months`) em `incidentCharts` e `prbCharts` | `S.incidentMonths`/`S.prbMonths` global era compartilhado por todos os charts do mesmo tipo — alterar num gráfico alterava todos; `chart.months` no objeto permite configuração independente com fallback para o global quando não definido; servidor recebe `Math.max(...todos os months)` para ter dados suficientes para qualquer gráfico |
| 171 | `preserveScroll` em `_load(refresh, preserveScroll)` — picker vs navegação | Pickers passam `(refresh) => _load(refresh, true)` via `initPickerCallbacks`, preservando `body.scrollTop` ao reconfigurar um gráfico; mudança de mês e refresh explícito chamam `_load` diretamente sem o flag e fazem scroll ao topo intencionalmente |
| 172 | `dashSource` em `localStorage` — última seleção AZ/SN persiste entre reloads | Segue o padrão já estabelecido de `dashView`, `snView` e `sidebarCollapsed`; `toggleSource` salva ao trocar; `app.js` restaura na inicialização chamando `toggleSource('sn')` apenas quando necessário (AZ já é o padrão do HTML renderizado pelo servidor) |
